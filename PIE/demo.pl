...
MAIN:
{
  ...
  # The following runs a check on the knowledge base. The check makes sure
  # all attributes are set by rules or questions. It also make sure that
  # all questions set attributes that are actually used. In other words,
  # it makes sure that all rules use attributes that have the possibility
  # to be set, and makes sure there are no redundant questions.
  # Once a knowledge base is setup and ready, remove this check.
  my @errors = $kb->check_kb;
  my $err;
  foreach $err (@errors) {
    print "Error: $err\n";
  }
  ...
  # to know about the rules in the Knowledge Base.
	my $engine=new Pie::Engine($kb);
